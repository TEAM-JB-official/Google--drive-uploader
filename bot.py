...
async def start_cmd(client, message: Message):
        ...
        if referrer and referrer["_id"] != user_id:
            ...
            if not existing or existing.get("referred_by") is None:
                ...
                if referral_count % 3 == 0 and referral_count > 0:
                    # Give 7 days premium (store as reward days)
    ...
    await message.reply(
        ...
        "• Set custom folder\n"
        "• Daily limits (free 4, premium 50)\n"
        "• Referral program\n\n"
        "Use /login to connect Google Drive\n"
...
/upload – Reply to a file or send direct link
/yt <YouTube URL> – Download and upload YouTube video
/setfolder <folder_url> – Set default upload folder
/removefolder – Remove custom folder
/myplan – Show your plan and usage
/upgrade – Info about premium
/stats – Your upload stats
/account – Your account details
/referral – Get your referral link
/mydrives – List connected Google accounts (not implemented fully)"""
...
async def login_cmd(client, message):
    ...
    await message.reply("Click the button below to connect your Google Drive:", reply_markup=kb)

# Handle file uploads (documents, video, audio, photo, voice)
...
# Handle direct links (HTTP/HTTPS) as command /upload <url> or just URL in text
...
async def upload_url_cmd(client, message):
    ...
    if not allowed:
        ...
        return
    folder_id = user.get("custom_folder_id")
    status_msg = await message.reply(f"⏳ Downloading from URL...")
    os.makedirs("downloads", exist_ok=True)
    file_path = f"downloads/{user_id}_{uuid.uuid4()}_{filename}"
    try:
...
async def set_folder_cmd(client, message):
    ...
    await message.reply("✅ Default upload folder set. All future uploads will go there.")
...
async def remove_folder_cmd(client, message):
    ...
    await message.reply("✅ Custom folder removed. Uploads will go to Drive root.")
...
async def myplan_cmd(client, message):
    ...
    if rewards > 0:
        text += "\n\nType /upgrade to activate referral rewards."
    await message.reply(text)
...
async def upgrade_cmd(client, message):
    ...
    rewards = user.get("referral_rewards", 0)
    if rewards > 0:
        # Activate premium for rewards days
        # For simplicity, set plan to premium. In real life, track expiry.
        await users_col.update_one({"_id": user_id}, {"$set": {"plan": "premium", "referral_rewards": 0}})
        await message.reply(f"🎉 You have been upgraded to Premium for {rewards} days! Enjoy 50 daily uploads.")
    else:
        await message.reply("Premium plans coming soon. For now, invite friends using /referral to get free premium days.")

@app.on_message(filters.command("stats"))
async def stats_cmd(client, message):
    user_id = message.from_user.id
    # Aggregate from logs
    total_uploads = await users_col.aggregate([
        {"$match": {"user_id": user_id, "action": "upload", "status": "success"}},
        {"$count": "count"}
    ]).to_list(length=1)
    total_uploads = total_uploads[0]["count"] if total_uploads else 0
...
async def referral_cmd(client, message):
    ...
    link = f"https://t.me/{app.me.username}?start=ref_{code}"
