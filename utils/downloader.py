...
# ========== HTTP downloader (streaming, 2 MB chunks) ==========
async def download_http(url, dest_path, progress_callback=None):
    """
    Download a file from an HTTP/HTTPS URL with streaming.
...
async def download_youtube(url, dest_template, progress_callback=None):
    """
    YouTube downloader with true streaming when possible.
    Falls back to yt-dlp merge (disk‑based) if needed.
    """
    ...
    if direct_fmt:
        ...
        # use the same streaming download_http
        await download_http(direct_url, final_path, progress_callback)
        return final_path

    # 3. No direct format – fallback to yt-dlp built‑in (uses ffmpeg merge)
